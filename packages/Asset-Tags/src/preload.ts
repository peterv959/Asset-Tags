...
contextBridge.exposeInMainWorld('electron', {
    ...
    app: {
        isDemoMode: () => process.env.VITE_APP_MODE === 'demo',
    },
    label: {
    ...
    menu: {
        onSelectTemplate: (callback: (configName: string) => void) =>
            ipcRenderer.on('select-template', (_event, configName: string) => callback(configName)),
        onOpenTemplateSelectorDialog: (callback: () => void) =>
            ipcRenderer.on('open-template-selector', () => callback()),
        onOpenConsecutivePrintDialog: (callback: () => void) =>
            ipcRenderer.on('open-consecutive-print-dialog', () => callback()),
        onOpenPrinterDialog: (callback: () => void) =>
            ipcRenderer.on('open-printer-dialog', () => callback()),
    },
});
