...
import { ConsecutivePrintDialog } from './ConsecutivePrintDialog';
...
export const App: React.FC = () => {
    ...
    const [previewZpl, setPreviewZpl] = React.useState<string | string[] | null>(null);
    const [showConsecutivePrintDialog, setShowConsecutivePrintDialog] = React.useState(false);
    const [showTemplateSelector, setShowTemplateSelector] = React.useState(false);
    const [showPrinterDialog, setShowPrinterDialog] = React.useState(false);
    const [consecutivePrintCount, setConsecutivePrintCount] = React.useState(1);
    ...
    React.useEffect(() => {
        try {
            const inDemoMode = (window.electron as any).app.isDemoMode();
            setIsDemoMode(inDemoMode);
        } catch (err) {
    ...
    React.useEffect(() => {
        const unsubscribeTemplate = (window.electron as any).menu.onSelectTemplate((configName: string) => {
            setConfigName(configName);
            const saveConfig = async () => {
                try {
                    const result = await (window.electron as any).appDefaults.load();
                    const defaults = result.defaults || {};
                    defaults.lastSelectedConfigName = configName;
        ...
        const unsubscribeTemplateDialog = (window.electron as any).menu.onOpenTemplateSelectorDialog(() => {
            setShowTemplateSelector(true);
        });

        const unsubscribeConsecutivePrint = (window.electron as any).menu.onOpenConsecutivePrintDialog(() => {
            setShowConsecutivePrintDialog(true);
        });
        const unsubscribePrinterSettings = (window.electron as any).menu.onOpenPrinterDialog?.(() => {
    ...
    const handleConsecutivePrint = async (count: number) => {
        ...
        setIsLoading(true);
        const collectedZpls: string[] = [];
        try {
            // Parse the starting asset tag as a number
            const startingAssetTag = parseInt(formData.assetTag, 10);

            for (let i = 0; i < count; i++) {
                // Increment the asset tag for each label
                const currentAssetTag = (startingAssetTag + i).toString();

                const labelDataWithIncrementedTag = {
                    ...formData,
                    assetTag: currentAssetTag,
                };

                const result = await window.electron.label.printLabel(labelDataWithIncrementedTag, {
                    host: printer.host,
                    port: printer.port || 9100,
                });

                if (!result.success) {
                    setError({ show: true, message: `Failed to print label ${i + 1} (asset tag ${currentAssetTag}): ${result.message}` });
                    break;
                }

                // In demo mode, collect the ZPL
                if (result.isDemo && result.zpl) {
                    collectedZpls.push(result.zpl);
                }
            }

            // If we collected any ZPL in demo mode, show them all
            if (collectedZpls.length > 0) {
                setPreviewZpl(collectedZpls);
            }

            setShowConsecutivePrintDialog(false);
            // Focus back to asset tag input
            ...
            setError({
                show: true,
                message: err instanceof Error ? err.message : 'Failed to print labels',
            });
        } finally {
            setIsLoading(false);
        }
    ...
    const handlePrint = async () => {
        ...
        try {
            ...
            if (!result.success) {
                ...
                // In demo mode, show the ZPL preview modal
                if (result.isDemo && result.zpl) {
                    setPreviewZpl(result.zpl);
                } else {
                    // In production mode, focus back to asset tag input and select all text
                    if (assetTagInputRef.current) {
                        assetTagInputRef.current.focus();
                        assetTagInputRef.current.select();
                    }
    ...
    return React.createElement(
        ...
        React.createElement(
            ...
            React.createElement(
                ...
                React.createElement(LabelForm, {
                    ...
                    disableSerialNumber: showConsecutivePrintDialog,
                })
            ),
            React.createElement(
                'div',
                ...
                React.createElement(LabelPreview, {
                    ...
                    isLoading,
                    printButtonRef,
                    selectedPrinterName: selectedPrinterId !== null ? printers[selectedPrinterId]?.name : undefined,
                })
            )
        ),
        error.show
            ...
            : null,
        previewZpl
            ? React.createElement(ZPLPreviewModal, {
                zpl: previewZpl,
                count: Array.isArray(previewZpl) ? previewZpl.length : undefined,
                onClose: () => {
                    setPreviewZpl(null);
                    // Focus back to asset tag input after closing modal
                    if (assetTagInputRef.current && !Array.isArray(previewZpl)) {
                        assetTagInputRef.current.focus();
                        assetTagInputRef.current.select();
                    }
                },
            ...
            : null,
        isDemoMode ? React.createElement(
            'div',
            { className: 'demo-mode-banner' },
            '🔧 DEMO MODE - ZPL will be displayed instead of sent to printer'
        ) : null,
        showConsecutivePrintDialog
            ? React.createElement(ConsecutivePrintDialog, {
                onPrint: handleConsecutivePrint,
                onClose: () => setShowConsecutivePrintDialog(false),
                isLoading,
                serialNumber: formData.serialNumber,
            })
            : null,
        showTemplateSelector
            ...
            : null,
        showPrinterDialog
            ? React.createElement(PrinterSelectionDialog, {
                printers,
                selectedPrinterId,
                isLoading,
                isTesting,
